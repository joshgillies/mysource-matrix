 ...
 * +--------------------------------------------------------------------+
 *
 * $Id: system_integrity_invalid_remaps.php,v 1.1.2.2 2013/09/06 03:01:51 ewang Exp $
 */
...
if(getCLIArg('redirect_chain')) {
    ...
    $sql = '(SELECT  a.* FROM sq_ast_lookup_remap a, sq_ast_lookup_remap b WHERE a.url = b.remap_url)  UNION ALL (SELECT  c.* FROM sq_ast_lookup_remap c, sq_ast_lookup_remap d WHERE c.remap_url = d.url except SELECT  c.* FROM sq_ast_lookup_remap c, sq_ast_lookup_remap d WHERE c.url = d.remap_url);';
    $query = MatrixDAL::preparePdoQuery($sql);
    $result =  MatrixDAL::executePdoAll($query);
    $sorted_result = Array();
...
function recursiveTrace ($url, &$result, $trace_order)
{
	...
	else if (!$trace_order && $url_info['remap_url'] ===  $url) {
	     unset($result[$index]);
