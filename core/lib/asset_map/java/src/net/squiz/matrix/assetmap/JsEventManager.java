...
* +--------------------------------------------------------------------+
*
* $Id: JsEventManager.java,v 1.1 2004/01/13 00:51:11 mmcintyre Exp $
...
public class JsEventManager {
	...
	*/
	private static final String JS_EVENT_CLASS = "net.squiz.matrix.assetmap.JsEvent";
	...
	*/
	...
	/**
	* adds a <code>AssetFinderListener</code> object to the list of listeners
	*
	* @param listener the <code>JsListener</code> object signing up to listen
	*/
	public void addJsListener(String type, AssetFinderListener l) {
	...
	*/
	...
	}//end getListenersByType()


	/**
	* Returns the Javascript method name that should be called for a specific event
	* 
	* @param eventName the name of the event that the method name is wanted for
	*
	* @return the name of the javascript method, or null if there is no method registered for this event
	*/
	...
	* @param params the params being passed from the javascript (as a matrix serialised String)
	*/
	public void jsToJavaCall(String type, String command, String paramsStr) throws JsEventNotFoundException {
		...
		// convert the matrix js serialised array (string) into something useful for java
		try {
		...
		if (!(listenerList.containsKey(type.toLowerCase().trim()))) {
			throw new JsEventNotFoundException("There are no " + command + " type listeners listening");
		}
		...
		for (Enumeration e = listeners.elements(); e.hasMoreElements();) {
			...
			try {
				// get the method for this particular event, based on the command name
				eventHandler = l.getClass().getDeclaredMethod(command, new Class [] {Class.forName(JsEventManager.JS_EVENT_CLASS)} );
			} catch (NoSuchMethodException nsme) {
				System.err.println("There is no event " + command + ": " + nsme.getMessage());
				return;
			} catch (ClassNotFoundException cnfe) {
				System.err.println("Could not find the event class " + cnfe.getMessage());
				return;
			}//end try/catch
			...
			try {
				...
				System.err.println("Illegal access exception when calling method " + command + ": " + iae.getMessage());
			} catch (IllegalArgumentException iare) {
				System.err.println("Illegal Argument Exception when calling method " + command + ": " + iare.getMessage());
			} catch (InvocationTargetException ite) {
				System.err.println("Invocation Target Exception when calling method " + command + ": " + ite.getTargetException().getMessage());
			}//end try/catch
	...
	*/
	...
	/**
	* Unpacks a matrix serialized <code>javascript</code> array into a <code>HashMap</code>
	*
	...
	*/
	public Map unpackJsArray(String data) throws IOException {
		...
		try {
			...
			throw new IOException("Could not parse the javascript xml Array: " + se.getMessage());
		}
		...
		if (!(type.getFirstChild().getNodeValue().equals("array")))
			throw new IOException("Expected array for val_type, got: " + type.getFirstChild().getNodeValue());
