 ...
 * </p>
 *
 * <p><code>$Id: AssetFactory.java,v 1.1 2003/11/14 05:21:36 dwong Exp $</code></p>
 *
...
public class AssetFactory {
	...
	private SortedMap assets;
	/** map of linkids to <code>AssetLink</code> objects */
	private SortedMap links;


	/** 
	 * Constructor. Private to ensure singleton instance.
	 */
	 ...
	 * @throws IOException		If a communication error occurred, e.g. XML parse 
	 *							error, error returned by the server, no connection
	...
	public void updateAsset(Asset parent) throws IOException {
		...
		while (childAssets.hasNext()) {
			...
			if (!childAsset.loaded()) {
				count++;
				request += "<asset assetid=\"" + childAsset.id() + "\" />";
			}
		}
		request += "</command>";
	 ...
	 * @throws IOException		A parse error occurred, or an asset references 
	 *							an unknown <code>AssetType</code>
	 */
	 ...
	 *
	 * @param assetElement 
	 * @param assetLinks		links found for this asset are put here for 
	 *							later processing by processAssetsElement
	 ...
	 */
	private void processAssetElement(Element assetElement, Map assetLinks) 
		...
		assetLinks.put(new Integer(asset.id()), linkElements);
	...
	private void processLinkElements(Asset parent, NodeList linkElements) {
		...
		for (int i = 0; i < newLinks.length; ++i) {
			...
			links.put(new Integer(link.id()), link);
		}
	 ...
	 * 
	 * @param assetid			the asset ID
	 * @param createIfMissing	whether to create a temporary 
	 *							<code>Asset</code> that contains the ID only
	 * @return					the <code>Asset</code> for the given ID
	 */
	public Asset getAsset(int assetid, boolean createIfMissing) {
		Integer assetidInteger = new Integer(assetid);
		Asset asset = null;
		if (!assets.containsKey(assetidInteger)) {
			if (createIfMissing) {
				// make a temporary asset without any info
				asset = new Asset(assetid);
				assets.put(assetidInteger, asset);
			}
		} else {
			asset = (Asset)assets.get(assetidInteger);
		}
