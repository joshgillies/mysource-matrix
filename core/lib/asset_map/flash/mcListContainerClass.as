...
* +--------------------------------------------------------------------+
*
* $Id: mcListContainerClass.as,v 1.44 2003/11/18 15:37:35 brobertson Exp $
...
mcListContainerClass.prototype.showActionsBar = function() 
{
	...
	if (this._parent.finding_asset) {
		...
		if (this.list.selected_item.preview_url != '') {
...
//			trace("finish asset finder");
		...
		if (action == 'preview') {
			...
			link = link.replace("%asset_path%", escape(this.list.selected_item.asset_path.join(",")));
			link = link.replace("%link_path%", escape(this.list.selected_item.link_path.join(",")));
