...
case"integer":case"double":str+='<val_type>'+type+'</val_type><val>'+value+'</val>\n';break;case"null":str+='<val_type>'+type+'</val_type><val></val>\n';break;case"boolean":str+='<val_type>'+type+'</val_type><val>'+((value)?1:0)+'</val>\n';break;case"array":str+='<val_type>'+type+'</val_type>\n';for(var k in value){var sub_str=_var_serialise(value[k],indent+' ');if(sub_str==false)return false;if(VAR_SERIALISE_STRING_ESCAPE_CHAR_REG_EXP.test(k)){alert(js_translate('data_contains_escape_character',VAR_SERIALISE_STRING_ESCAPE_CHAR.charCodeAt(0)));k=k.replace(VAR_SERIALISE_STRING_ESCAPE_CHAR_REG_EXP,'');}
...
if(str=='<val_type>array</val_type>'){var indent_len=indent.length;VAR_UNSERIALISE_I++;var val=new Array();for(var key in val)val[key]=null;while(lines[VAR_UNSERIALISE_I]!=null&&indent+' '==lines[VAR_UNSERIALISE_I].substr(0,indent_len+1)){var results=_var_unserialise(lines,indent+' ');val[results[1]]=results[0];VAR_UNSERIALISE_I++;}
...
return new_str;}
function sq_print_icon(path,width,height,alt){var is_ie10=(navigator.userAgent.toLowerCase().indexOf("msie 10")!=-1);if((typeof window.ActiveXObject!="undefined")&&!is_ie10){document.write('<span style="height:'+height+'px;width:'+width+'px; filter:progid:DXImageTransform.Microsoft.AlphaImageLoader (src=\''+path+'\', sizingMethod=\'scale\')" title="'+alt+'"></span>');}else{document.write('<img src="'+path+'" width="'+width+'" height="'+height+'" border="0" alt="'+alt+'" />');}}
...
function show(){if(is_ie4up){var top_offset=20;if(document.getElementById('sq-content')){scroll_top_content=document.getElementById('sq-content').scrollTop;scroll_top_body=document.body.scrollTop;if(scroll_top_content<=scroll_top_body){scroll_top=scroll_top_body;}else{scroll_top=scroll_top_content;}
this.move(null,top_offset+scroll_top);}else{scroll_top=document.documentElement.scrollTop;this.move(null,top_offset+scroll_top);}}
this.style.visibility=(is_nav4)?"show":"visible";}
function hide(){this.style.visibility=(is_nav4)?"hide":"hidden";}
...
SQ_FORM_ERROR_CONTAINED=false;inputs=f.getElementsByTagName('input');for(i=0;i<inputs.length;i++){if(inputs[i].name=='submit'){SQ_FORM_ERROR_CONTAINED=true;}}
...
{if(arguments.length<2)return false;if(parseInt(name.value)!=name.value||(name.value.length>0&&allow_negative==false&&name.value.charAt(0)=="-")){var outstr="";for(ii=0;ii<name.value.length;ii++){if((parseInt(name.value.charAt(ii))==name.value.charAt(ii))||(allow_negative==true&&ii==0&&name.value.charAt(ii)=="-")){outstr+=name.value.charAt(ii);}}
...
newHiddenField=hiddenField.cloneNode(true);newHiddenField.value='0';newCheckbox=checkbox.cloneNode(true);newCheckbox.checked=0;newText=document.createTextNode('inherit ');elt.parentNode.insertBefore(newHiddenField,elt);elt.parentNode.insertBefore(newCheckbox,elt);elt.parentNode.insertBefore(newText,elt);}
...
function bodycopy_delete_div(bodycopy_name,divid){if(confirm('Are you sure you want to delete this DIV?')){var data=new Object();data["containerid"]=divid;bodycopy_submit("delete_container",bodycopy_name,data);}}
...
bodycopy_show_popup("edit_div_conditions.php",380,470);}
