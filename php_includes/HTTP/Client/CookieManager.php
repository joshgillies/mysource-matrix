...
// +----------------------------------------------------------------------+
//
// $Id: CookieManager.php,v 1.1.4.1 2006/09/25 05:55:49 arailean Exp $
	...
	*/
	function passCookies(&$request)
	{
		if (!empty($this->_cookies)) {
			$url =& $request->_url;
			$url_path = empty($url->path) ? '/' : $url->path;
			...
			foreach ($this->_cookies as $cookie) {
				if ($this->_domainMatch($url->host, $cookie['domain']) && (0 === strpos($url_path, $cookie['path']))
